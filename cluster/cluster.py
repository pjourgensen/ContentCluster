'''
This script uses a generated data set (from a .json file) and runs a specified clustering algorithm on it
...
#cost function => compute square difference of each genre bit
def square_diff(datapoint, mean):
    cost = 0
    for x in range(len(datapoint["genres"])):
        cost = cost + (abs(pow((datapoint["genres"][x] - mean[x]),2)))
    return cost

#cost function => compute positive cubic difference of each genre bit
def cubic_diff(datapoint, mean):
    cost = 0
    for x in range(len(datapoint["genres"])):
        cost = cost + (abs(pow((datapoint["genres"][x] - mean[x]),3)))
    return cost
...
func_list = {"directCompare": direct_compare, "directDiff": direct_diff, "squareDiff": square_diff, "cubicDiff": cubic_diff, "arithMean": arith_mean, "intraErrorOnly": intra_error_only, "diffError": diff_error, "clusterError": intra_cluster_size, "probClusterError": prob_intra_cluster_size}
dist = None
...
algorithm = ""
alg_options = {}
with open('../config/cluster_config.json') as json_file:  
